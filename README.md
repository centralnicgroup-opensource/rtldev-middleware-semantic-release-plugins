...
The `notes` value can also be supplied via the `customReleaseNotes` environment variable, which takes precedence over the plugin config. Internal commit, issue, and pull-request links are stripped from the output before it is returned to semantic-release; documentation and other links are preserved.
