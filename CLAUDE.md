...
- **Release notes, and who owns which half:** `notes-override` publishes a string produced elsewhere (a Jira query, a pipeline-written changelog, a distribution repository republishing what its source rendered) and never looks at commits — it is the single source for that replay path and is used by repositories with no scope catalogue at all. Rendering notes _from_ commits belongs to `whmcs-build`'s `releaseNotes` option, not to a plugin of its own. Do not teach either side the other's job.
- **`whmcs-build` release notes (`notes/`):** with `releaseNotes` configured the plugin implements `generateNotes` and wraps `@semantic-release/release-notes-generator` rather than replacing it. Structure to preserve:
  - `ScopeCatalogue` (`notes/scope-catalogue.js`) owns the whole vocabulary: loading, optional-word expansion, the exact index, resolution, and `check()`. It is the single source consumers reuse for commit linting and CI checks - do not grow a second resolver next to it. Lookups stay **exact**: product directory names collide in practice (`cnicdns` is a prefix of `cnicdnsmanager`), so a fuzzy match would silently print the wrong product in a customer-facing note. Unknown scopes get acronym-aware title case and are reported.
  - `ReleaseNotesRenderer` (`notes/renderer.js`) turns a catalogue plus an audience into the generator's parser/writer options; `notes/render.js` is the one entry point both `generateNotes` and `DistributionRepoPublisher` call, so the downstream `customer` cut can never drift from the release's own notes. `notes/preview.js` renders the same options against `git log` for a dry look at the next release.
  - `notes/templates.js` holds Handlebars template **strings**. conventional-changelog-writer@8 registers partials through Handlebars; the Angular preset's own partials became render functions in v9 and render an empty commit list if passed here. Anything needing logic is precomputed in `transform()`.
  - Note titles are restored from the parsed commit by position, never taken from the preset: Angular v8's writer maps _every_ note title to BREAKING CHANGES, so an `UPGRADE NOTES:` trailer would be filed as a breaking change on one preset version and not on another.
  - The brand vocabulary (`optionalWords`, `acronyms`, the entries) lives in the consumer's catalogue file, not in this package. Keep it that way.
  - `@semantic-release/release-notes-generator` and `conventional-changelog-angular` are optional peers reached through `await import(...)`, like every other optional peer here.
